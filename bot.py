...
async def handle_year(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    save_past_question(
        ...
        year, update.message.from_user.username
    )
...
async def getpast(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("📚 Enter the course name:")
    return GET_COURSE
...
async def get_year(update: Update, context: ContextTypes.DEFAULT_TYPE):
    course, level, year = context.user_data["course"], context.user_data["level"], update.message.text
    results = get_past_questions(course, level, year)
    if not results:
